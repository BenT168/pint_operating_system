...
/* Has the same functionality as 'thread_yield', except for the fact that the
   current thread is passed as an argument, rather than returned by code in the
   function. Functions that call 'thread_yield_current', want to ensure that the
   thread being yielded will be 'cur', and not some other thread returned by
   'thread_current ()'. A situation which can occur due to context switching
...
{
  /* We must avoid multiple calls to 'thread_current ()' when changing a
     thread's priority. This is because, if an interrupt occurs, successive
     calls to 'thread_current ()' may return different threads, which of course
  ...
  {
    /* An invariant on the list of 'READY' threads, is that it
       is sorted in ascending order of the priorities of the threads. Hence,
       the last element of the list will be the thread with the highest
       priority. */
